...
I = cv2.imread("/home/noemi/Documents/Suli/Gépi_látás/Valos_tablak/kep38.jpg")
# A keresett minták
#melsobbseg = cv2.imread("/home/noemi/Documents/Suli/Gépi_látás/Tablak_minta/elsobbsegadas.png")
#mstop = cv2.imread("/home/noemi/Documents/Suli/Gépi_látás/Tablak_minta/stop.jpg")
...
mfout = cv2.imread("/home/noemi/Documents/Suli/Gépi_látás/Tablak_minta/foutvonal.png")
			...
			if len(approx) == 3:
				...
				for wcnt in wcontours:
					...
					if warea>500:
						...
						#print(len(wapprox), 'white')
						if len(wapprox) == 3:
								#cv2.imshow('fehermaszk',whitemask)
...
for cnt2 in contours2:
	...
	if area>500:
		...
		if len(approx) == 4 or len(approx) == 8:
			x, y, w, h = cv2.boundingRect(approx)
			#cv2.rectangle(I, (x - 20, y - 20), (x + w + 20, y + h + 20), (0,0,0), 5)
			dim = (w, w)
...
for cnt3 in contours3:
	...
	if area>500:
		...
		if len(approx) == 4:
			x, y, w, h = cv2.boundingRect(approx)
			#cv2.rectangle(I, (x - 50, y - 50), (x + w + 50, y + h + 50), (255,255,0), 5)
			dim = (w+50, w+50)
			if len(approx) == 4:
				mfout = cv2.resize(mfout, dim, interpolation = cv2.INTER_AREA)
				M = cv2.matchTemplate(I, mfout, cv2.TM_CCOEFF)
				(_, _, _, maxloc) = cv2.minMaxLoc(M)
				cv2.rectangle(I, (maxloc[0]-20, maxloc[1]-20),  (maxloc[0] +  mfout.shape[1]+20,  maxloc[1] + mfout.shape[0]+20), (255, 0, 255), 3)
				#print('maxloc',maxloc[0],'x',x)
				if abs(maxloc[0]-x) < 50 :
